...
const NavigationTab: React.FC<NavigationTabProps> = ({ title = '', active, onPress }) => {
    return (
        <TouchableOpacity style={tabStyle.container}>
            <Text style={[tabStyle.text, active ? tabStyle.textActive : tabStyle.textInactive]} onPress={() => Alert.alert({te})}>{title}</Text>
        </TouchableOpacity>
    );
};
...
export const BottomNavigationBar: React.FC = () => {
    return (
        <View style={style.bar}>
            ...
            <NavigationTab title="Home" active={false} onPress= {() => Alert.alert('Help')}/>
